...
lines = [x for x in puzzleInput.split('\n')]
paths = [line.split(',') for line in lines]

def inc_grid(grid, intersections, wire_no, step_count, x, y):
    if x not in grid:
        grid[x] = {}
    if y not in grid[x]:
        grid[x][y] = {}
...
dists = [calc_steps(steps) for x, y, steps in intersections]
